...
const errorHandlerPipe = (message: string) => {
  return (details: any) =>
    of(DashboardActions.throwError({ message, details }));
};
...
export class DashboardEffects {
  ...
  public updateGoal$ = createEffect(() =>
    this.actions$.pipe(
      ofType(DashboardActions.updateGoal),
      concatMap((_action) =>
        this.goalService.updateGoal(_action.goal).pipe(
  ...
  public createGoal$ = createEffect(() =>
    this.actions$.pipe(
      ofType(DashboardActions.createGoal),
      concatMap((_action) =>
        this.goalService.createGoal(_action.goal).pipe(
  ...
  public createComment$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((_action) =>
        this.goalService.createComment(_action.comment).pipe(
  ...
  public createReplyComment$ = createEffect(() =>
    this.actions$.pipe(
      ofType(DashboardActions.createReplyComment),
      concatMap((_action) =>
        this.goalService.createReplyComment(_action.replyComment).pipe(
          map((goal) => DashboardActions.createCommentSuccess({ goal })),
          catchError((error: Error) => {
            console.log(error);
            return errorHandlerPipe(error.message);
  ...
  public deleteComment$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap((_action) =>
        this.goalService.deleteComment(_action.commentId).pipe(
          map((commentId) =>
            DashboardActions.deleteCommentSuccess({ commentId })
          ),
          catchError((error: Error) => {
            console.log(error);
            return errorHandlerPipe(error.message);
